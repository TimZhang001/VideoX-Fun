...
def load_model(args, config, device):
    ...
    return pipeline, generator, vae, transformer, config
...
def main(args):
    ...
    # 模型加载
    pipeline, generator, vae, transformer, config = load_model(args, config, device)
    ...
    with torch.no_grad():
        ...
        for prompt in prompt_list:
            ...
            if transformer.config.in_channels != vae.config.latent_channels:
                input_video, input_video_mask, _ = get_image_to_video_latent(None, None, video_length=video_length, sample_size=args.sample_size)
